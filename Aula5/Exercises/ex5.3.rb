...
# Seu código aqui...
  puts people.reject { |x| x[:gender] != :male }.size()
# 2) Quantas são mulheres?

# Seu código aqui...
  puts people.reject { |x| x[:gender] != :female }.size()

# 3) Quantas pessoas são maiores de idade?

# Seu código aqui...
  puts people.reject { |x| x[:age] > 18 }.size()

# 4) Qual a soma das idades de todos os brasileiros?

# Seu código aqui...
  puts people.reduce(0) { |sum,x| sum + x[:age]}

# 5) Imprima todos os nomes em ordem alfabética

# Seu código aqui...
  v = []
  people.each { |x| v << x[:name]}
  puts v.sort
